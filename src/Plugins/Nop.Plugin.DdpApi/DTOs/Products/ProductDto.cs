...
namespace Nop.Plugin.DdpApi.DTO.Products
{
    ...
    public class ProductDto : BaseDto
    {
        ...
        public void Merge(ref Product product)
        {
            product.Name = Name;
            product.Sku = Sku;
            product.Price = Price ?? decimal.Zero;
            product.ShortDescription = ShortDescription;
            product.Published = Published.HasValue && Published.Value;
            
            product.StockQuantity = StockQuantity ?? 0;
            
            product.Gtin = Gtin;
            product.IsTaxExempt = IsTaxExempt.HasValue && IsTaxExempt.Value;

            product.ManageInventoryMethod = ManageInventoryMethodId.HasValue ? (ManageInventoryMethod)ManageInventoryMethodId : ManageInventoryMethod.DontManageStock;
        }
    }
}