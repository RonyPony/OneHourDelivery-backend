...
namespace Nop.Plugin.DdpApi.ModelBinders
{
    ...
    public class JsonModelBinder<T> : IModelBinder where T : class, new()
    {
        ...
        private void ValidateValueTypes(ModelBindingContext bindingContext, Dictionary<string, object> propertyValuePaires)
        {
            ...
            if (!typeValidator.IsValid(propertyValuePaires))
            {
                ...
                {
                    var key = string.Format(_localizationService.GetResource("DdpApi.InvalidType", _languageId, false), invalidProperty);

                    if (!errors.ContainsKey(key))
                    {
                        errors.Add(key, _localizationService.GetResource("DdpApi.InvalidPropertyType", _languageId, false));
                    }
                }
            }
