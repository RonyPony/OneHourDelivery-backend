...
namespace Nop.Plugin.DdpApi.Helpers
{
    public class JsonHelper : IJsonHelper
    {
        ...
        public Dictionary<string, object> GetRequestJsonDictionaryFromStream(Stream stream, bool rewindStream)
        {
            ...
            {
                throw new InvalidOperationException(_localizationService.GetResource("DdpApi.NoJsonProvided", _languageId, false));
            }

            var requestBodyDictioanry = DeserializeToDictionary(json);
            if (requestBodyDictioanry == null || requestBodyDictioanry.Count == 0)
            {
                throw new InvalidOperationException(_localizationService.GetResource("DdpApi.InvalidJsonFormat", _languageId, false));
            }
