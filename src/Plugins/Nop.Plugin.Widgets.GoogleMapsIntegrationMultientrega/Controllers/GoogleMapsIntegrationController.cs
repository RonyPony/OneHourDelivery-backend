...
namespace Nop.Plugin.Widgets.GoogleMapsIntegrationMultientrega.Controllers
{
    ...
    public class GoogleMapsIntegrationController : BasePluginController
    {
        ...
        private bool GeoCoordinateIsValid(string geoCoordinate)
        {
            const int SPLITTED_DECIMAL_MAX_QTY_NUMBERS = 2, RIGHT_DECIMAL_NUMBER_MAX_QTY = 3, LEFT_DECIMAL_NUMBER_MAX_QTY = 7;

            string[] coordinateNumbers = geoCoordinate.Split(".");
            if (coordinateNumbers.Length > SPLITTED_DECIMAL_MAX_QTY_NUMBERS) return false;
            if (coordinateNumbers[0].Length > RIGHT_DECIMAL_NUMBER_MAX_QTY) return false;
            if (coordinateNumbers[1].Length > LEFT_DECIMAL_NUMBER_MAX_QTY) return false;
        ...
        public virtual IActionResult AddressAdd(AddressGeoCoordinatesEditModel model, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                address.City = _multientregaAddressService.GetProvinceNameById(model.AddressGeoCoordinatesMapping.ProvinceId);
                address.StateProvinceId = _multientregaAddressService.GetStateProvinceIdByProvinceId(model.AddressGeoCoordinatesMapping.ProvinceId);
        ...
        public virtual IActionResult AddressEdit(int addressId)
        {
            ...
            var model = new AddressGeoCoordinatesEditModel {
                PluginConfigurationSettings = _settingService.LoadSetting<PluginConfigurationSettings>(),
                AddressGeoCoordinatesMapping = mapping,
                RenderGoogleMapsJavaScript = true,
                MapDivId = "map",
                AutocompleteInputId = "autocomplete",
                GeoCoordinatesSearchInputId = "geocoordinatesSearch",
                ...
                AvailableDistricts = mapping != null ? _multientregaAddressService.GetDistrictsSelectListItems(mapping.ProvinceId) :
                        _multientregaAddressService.GetDistrictsSelectListItems("0"),
                AvailableTownships = mapping != null ? _multientregaAddressService.GetTownshipsSelectListItems(mapping.DistrictId) :
                        _multientregaAddressService.GetTownshipsSelectListItems("0"),
                AvailableNeighborhoods = mapping != null ? _multientregaAddressService.GetNeigborhoodsSelectListItems(mapping.TownshipId) :
        ...
        public virtual IActionResult NewBillingAddress(CheckoutBillingGeoCoordinatesAddressModel model, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (address == null)
                {
                    ...
                    address.CustomAttributes = customAttributes;
                    address.CreatedOnUtc = DateTime.UtcNow;
                    address.City = _multientregaAddressService.GetProvinceNameById(model.AddressGeoCoordinatesEditModel.AddressGeoCoordinatesMapping.ProvinceId);
                    address.StateProvinceId = _multientregaAddressService.GetStateProvinceIdByProvinceId(model.AddressGeoCoordinatesEditModel.AddressGeoCoordinatesMapping.ProvinceId);
                    //some validation
                    if (address.CountryId == 0)
                        address.CountryId = null;
                    if (address.StateProvinceId == 0)
        ...
        /// <summary>
        /// Updates an order address latitude and longitude.
        /// </summary>
        /// <param name="mapping">An instance of <see cref="AddressGeoCoordinatesMapping"/>.</param>
        /// <returns>An implementation of <see cref="IActionResult"/>.</returns>
        [HttpPost]
        public IActionResult UpdateOrderAddressLatLng(AddressGeoCoordinatesMapping mapping)
        {
            try
            {
                ...
                if (!GeoCoordinateIsValid(mapping.Latitude.ToString())
                    || !GeoCoordinateIsValid(mapping.Longitude.ToString()))
                    throw new Exception(_localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.OrderAddress.InvalidLatLng"));

                AddressGeoCoordinatesMapping addressCoordinates = _addressGeoCoordinatesService.GetAddressGeoCoordinates(mapping.AddressId);

                if (addressCoordinates != null)
                {
                    addressCoordinates.Latitude = mapping.Latitude;
                    addressCoordinates.Longitude = mapping.Longitude;

                    _addressGeoCoordinatesService.InsertAddressGeoCoordinates(addressCoordinates, mapping.AddressId);
                }

                return Json(new { Error = false, Message = _localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.OrderAddress.LatLngUpdatedSuccessfuly") });
            }
            catch (Exception e)
            {
                _logger.Error($"{_localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.OrderAddress.ErrorUpdatingGeoCoordinates")} {e.Message}", e);
                return Json(new { Error = true, Message = $"{_localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.OrderAddress.ErrorUpdatingGeoCoordinates")} {e.Message}" });
            }
        }
