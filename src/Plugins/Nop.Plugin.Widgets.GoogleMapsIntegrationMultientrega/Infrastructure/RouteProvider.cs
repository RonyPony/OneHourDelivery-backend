...
namespace Nop.Plugin.Widgets.GoogleMapsIntegrationMultientrega.Infrastructure
{
    ...
    public sealed class RouteProvider : IRouteProvider
    {
        ...
        public void RegisterRoutes(IEndpointRouteBuilder endpointRouteBuilder)
        {
            ...
            //get province list by country ID  (AJAX link)
            endpointRouteBuilder.MapControllerRoute("MultientregaGetProvinces", $"{pattern}multientrega/getprovincesbycountryid/",
                new { controller = "Multientrega", action = "GetProvincesByCountryId" });

            //get district list by province ID  (AJAX link)
            endpointRouteBuilder.MapControllerRoute("MultientregaGetDistricts", $"{pattern}multientrega/getdistrictsbyprovinceid/",
                new { controller = "Multientrega", action = "GetDistrictsByProvinceId" });

            //get township list by district ID  (AJAX link)
            endpointRouteBuilder.MapControllerRoute("MultientregaGetTownships", $"{pattern}multientrega/gettownshipsbydistrictid/",
                new { controller = "Multientrega", action = "GetTownshipsByDistrictId" });

            //get neighborhood list by township ID  (AJAX link)
            endpointRouteBuilder.MapControllerRoute("MultientregaGetNeighborhoods", $"{pattern}multientrega/getneighborhoodsbytownshipid/",
                new { controller = "Multientrega", action = "GetNeighborhoodsByTownshipId" });
        }
