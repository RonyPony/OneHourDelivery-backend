...
namespace Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Validators
{
    ...
    public sealed class RegisterCustomerRequestValidator : BaseNopValidator<RegisterCustomerRequest>
    {
            ...
            ICustomerAddressGeocodingServices customerAddressGeocodingServices)
        {
            ...
            RuleFor(x => x.Latitude).NotEmpty().WithMessage(localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.Fields.Latitude.Required"));
            RuleFor(x => x.Latitude).InclusiveBetween(-999.9999999m, 999.9999999m).WithMessage(localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.Fields.Longitude.NotValid"));

            RuleFor(x => x.Longitude).NotEmpty().WithMessage(localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.Fields.Longitude.Required"));
            RuleFor(x => x.Longitude).InclusiveBetween(-999.9999999m, 999.9999999m).WithMessage(localizationService.GetResource($"{Defaults.ResourcesNamePrefix}.Fields.Longitude.NotValid"));
        }
    }
}
