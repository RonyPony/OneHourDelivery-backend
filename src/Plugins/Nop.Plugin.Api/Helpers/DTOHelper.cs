    ...
    public class DTOHelper : IDTOHelper
    {
        ...
        public ProductDto PrepareProductDTO(Product product)
        {
            ...
            productDto.Tags = _productTagService.GetAllProductTagsByProductId(product.Id).Select(tag => tag.Name)
                .ToList();
            productDto.ProductAttributeMappings = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id).Select(attribute =>  new ProductAttributeMappingDto {
                ...
                ProductAttributeName= _productAttributeService.GetProductAttributeById(attribute.ProductAttributeId).Name,
                ProductAttributeValues = _productAttributeService.GetProductAttributeValues(attribute.Id).Select(value => new ProductAttributeValueDto
                {
        ...
        public OrderDto PrepareOrderDTO(Order order)
        {
            try
            {
                ...
                {
                    var pickUpInStoreAddress = _addressService.GetAddressById(order.PickupAddressId.Value);
