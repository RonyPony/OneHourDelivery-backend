...
namespace Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Mapping.Builders
{
    ...
    public partial class OrderPendingToClosePaymentItemBuilder : NopEntityBuilder<OrderPendingToClosePaymentItem>
    {
        ...
        {
            table
                .WithColumn(nameof(OrderPendingToClosePaymentItem.OrderId)).AsInt32().ForeignKey<OrderPendingToClosePayment>(onDelete: Rule.None)
                .WithColumn(nameof(OrderPendingToClosePaymentItem.ProductId)).AsInt32().ForeignKey<Product>(onDelete: Rule.None);
        }
