...
{
    public class DeliveryAppDtoHelper : IDTODeliveryAppHelper
    {
        ...
        public DeliveryAppDtoHelper(IProductService productService,
        ...
        public OrderDeliveryAppDto PrepareOrderDTO(Order order)
        {
            try
            {
                var orderDtoq = order.ToDto();
                var orderDto = BuildOrderDeliveryAppDtoFromOrderDto(orderDtoq);
                ...
                if (vendorId != 0)
                {
                    var vendorWarehouseRelation = _vendorWarehouseMappingRepository.Table.FirstOrDefault(x => x.VendorId == vendorId);

                    if (vendorWarehouseRelation == null)
                    {
                        throw new ArgumentException($"Vendor: {vendorId} does not have any warehouse associated.");
                    }

                    var warehouse = _warehouseRepository.Table.FirstOrDefault(x => x.Id == vendorWarehouseRelation.WarehouseId);

                    var vendor = _vendorService.GetVendorById(vendorId);
                    orderDto.VendorID = vendor.Id;
                    orderDto.VendorName = vendor.Name;
                    orderDto.VendorImage = _pictureService.GetPictureUrl(pictureId: vendor.PictureId);
                    orderDto.EstimatedDeliveryTime = GetEstimatedTimeAttribute(vendor.Id);

                    if (warehouse.AddressId != 0)
                    {
                        var vendorAddress = _addressService.GetAddressById(warehouse.AddressId);
        ...
        private OrderDeliveryAppDto BuildOrderDeliveryAppDtoFromOrderDto(OrderDto order)
        {
            var newOrderPendingToClosePayment = new OrderDeliveryAppDto();
        ...
        {
            return GetVendorAttributeValueByVendorIdAndAttributeName(vendorId, "Estimated wait time");
        }
        ...
        private string GetVendorAttributeValueByVendorIdAndAttributeName(int vendorId, string vendorAttributeName)
        {
            ...
            GenericAttribute vendorGenericAttributes = _genericAttributeService.GetAttributesForEntity(vendorId, "Vendor")
                .FirstOrDefault(x => x.Key == "VendorAttributes");
