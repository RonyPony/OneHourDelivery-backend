...
using Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Areas.Admin.Components;
using Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Components;
    ...
    public class DeliveryAppIntegrationPlugin : BasePlugin, IWidgetPlugin, IAdminMenuPlugin
    {
        ...
        private void InsertDeliveryAppRolesPermissions(IEnumerable<CustomerRole> customerRoles)
        {
            ...
            foreach (string roleName in Defaults.CustomerRolesPermissions.Keys)
            {
                ...
                if (currentCustomerRole != null)
                {
                    ...
                    {
                        PermissionRecord permission = allPermissions.FirstOrDefault(x => x.SystemName.Equals(rolePermissionName));
        ...
        private void InsertDefaultsShippingMethods()
        {
            foreach (KeyValuePair<string, decimal> keyValue in Defaults.ShippingMethodsByKm)
            {
                ...
                _settingService.SetSetting(string.Format(Defaults.FixedByWeightByTotalRateSettingsKey, newShippingMethod.Id), keyValue.Value, 0);
            }
        }
        ...
        {
            if (widgetZone == AdminWidgetZones.WarehouseListButtons || widgetZone == AdminWidgetZones.VendorDetailsBlock)
                return typeof(VendorWarehouseViewComponent);

            if (widgetZone == AdminWidgetZones.DiscountListButtons)
                return typeof(DeliveryAppDiscountComponent);

            if (widgetZone == AdminWidgetZones.DashboardTop)
                return typeof(DeliveryAppDashboardReportsComponent);
