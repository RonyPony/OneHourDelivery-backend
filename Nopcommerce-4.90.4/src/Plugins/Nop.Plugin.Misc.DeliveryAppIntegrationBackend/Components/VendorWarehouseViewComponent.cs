...
namespace Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Components
{
    ...
    public class VendorWarehouseViewComponent : NopViewComponent
    {
        ...
        public IViewComponentResult Invoke(string widgetZone, object additionalData)
        {
            try
            {
                if (widgetZone.Equals(AdminWidgetZones.WarehouseListButtons) && _vendorDeliveryAppService.IsDeliveryAppVendor(_workContext.GetCurrentCustomerAsync().GetAwaiter().GetResult()))
                {
                    VendorWarehouseMappingModel model = _vendorWarehouseMappingModelFactory.PrepareVendorWarehouseMappingModel(_workContext.GetCurrentCustomerAsync().GetAwaiter().GetResult().VendorId);
