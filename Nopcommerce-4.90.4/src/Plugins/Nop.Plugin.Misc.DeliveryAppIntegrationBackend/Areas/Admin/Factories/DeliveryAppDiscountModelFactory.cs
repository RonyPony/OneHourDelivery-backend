...
namespace Nop.Plugin.Misc.DeliveryAppIntegrationBackend.Areas.Admin.Factories
{
    ...
    public sealed class DeliveryAppDiscountModelFactory : IDeliveryAppDiscountModelFactory
    {
        ...
        public DeliveryAppDiscountSearchModel PrepareDiscountSearchModel(DeliveryAppDiscountSearchModel searchModel)
        {
            ...
            if (_workContext.GetCurrentVendorAsync().GetAwaiter().GetResult() != null) { searchModel.VendorId = _workContext.GetCurrentVendorAsync().GetAwaiter().GetResult().Id; }
        ...
        public DeliveryAppDiscountListModel PrepareDiscountListModel(DeliveryAppDiscountSearchModel searchModel)
        {
            ...
            var startDateUtc = searchModel.SearchStartDate.HasValue ?
                (DateTime?)_dateTimeHelper.ConvertToUtcTime(searchModel.SearchStartDate.Value, _dateTimeHelper.GetCurrentTimeZoneAsync().GetAwaiter().GetResult()) : null;
            var endDateUtc = searchModel.SearchEndDate.HasValue ?
                (DateTime?)_dateTimeHelper.ConvertToUtcTime(searchModel.SearchEndDate.Value, _dateTimeHelper.GetCurrentTimeZoneAsync().GetAwaiter().GetResult()).AddDays(1) : null;
        ...
        public AddProductToDiscountSearchModel PrepareAddProductToDiscountSearchModel(AddProductToDiscountSearchModel searchModel)
        {
            ...
            {
                searchModel.SearchVendorId = _workContext.GetCurrentVendorAsync().GetAwaiter().GetResult().Id;
                searchModel.AvailableVendors.Add(new SelectListItem { Text = _workContext.GetCurrentVendorAsync().GetAwaiter().GetResult().Name, Value = _workContext.GetCurrentVendorAsync().GetAwaiter().GetResult().Id.ToString() });
            }
        ...
        public DeliveryAppCustomerDiscountList GetCustomersAssignedToDiscountCoupon(int discountId)
        {
            ...
            foreach (CustomerDiscountMapping discount in foundCustomDiscount)
            {
                ...
                string firstName = _genericAttributeService.GetAttribute<string>(foundCustomer, "FirstName");
                string lastName = _genericAttributeService.GetAttribute<string>(foundCustomer, "LastName");
        ...
        public DeliveryAppAddCustomerToListModel PrepareAddAssignCustomerToDiscountSearchModel(DeliveryAppAddCustomerToDiscountSearchModel searchModel)
        {
            ...
            var model = new DeliveryAppAddCustomerToListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    customerModel.FirstName = _genericAttributeService.GetAttribute<string>(customer, "FirstName");
                    customerModel.LastName = _genericAttributeService.GetAttribute<string>(customer, "LastName");
